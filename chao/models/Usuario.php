...
class Usuario {
    ...
    public function readAll() {
        $query = "SELECT id_usuario, nombre, apellido, email, saldo FROM " . $this->table_name;
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt;
    }
    ...
    public function actualizarSaldo($monto) {
        $query = "UPDATE " . $this->table_name . " SET saldo = saldo + :monto WHERE id_usuario = :id_usuario";
        $stmt = $this->conn->prepare($query);
